...
function DataSourcesSection({ dataSourceType = 'team-skills', onDataProcessingComplete, onDataChanged }) {
  ...
  return (
    <Container
      header={
        <Header
          variant="h2"
          description={t('components:dataSources.manageDataSources', { dataType: 'team skills' })}
        >
          {t('components:dataSources.dataSources')}
        </Header>
      }
    >
      <SpaceBetween size="l">
        {uploadSuccess && (
          <Alert type="success" dismissible onDismiss={() => setUploadSuccess(false)}>
            {t('components:dataSources.fileUploadedSuccessfully')}
          </Alert>
        )}
        ...
        <Box textAlign="right">
          <Button 
            ...
            ariaLabel={
              !hasWriteAccess 
                ? t('components:dataSources.noPermissionToUpload')
                : !projectReady 
                  ? t('components:dataSources.projectNotReadyForUploads')
                  : t('components:dataSources.uploadNewFile')
            }
          >
            {t('components:dataSources.uploadNewFile')}
          </Button>
          ...
          {!projectReady && (
            <Box color="text-status-info" padding={{ top: 'xs' }}>
              <small>{t('components:dataSources.projectMustBeActive')}</small>
            </Box>
          )}
        </Box>
      </SpaceBetween>
      ...
      {selectedProject && projectReady && (
        <FileUploadModal
          ...
          projectId={projectId}
          folderPath={`data-uploaded/${dataSourceType}/`}
          dataSourceType={dataSourceType}
