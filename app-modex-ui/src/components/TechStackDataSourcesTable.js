...
function TechStackDataSourcesTable({ dataSourceType = 'applications-tech-stack', refreshTrigger = 0, projectId, onDataProcessingComplete, onDataChanged }) {
  ...
  const handleDownload = async (id, filename) => {
    try {
      ...
      if (!response.success) {
        ...
        addNotification('error', t('components:techStackDataSources.downloadError', { filename, error: response.error }));
      } else {
        console.log(`✅ Download initiated successfully for ${filename}`);
        addNotification('success', t('components:techStackDataSources.downloadInitiated', { filename }));
      }
    } catch (error) {
      console.error('Error downloading file:', error);
      addNotification('error', t('components:techStackDataSources.downloadError', { filename }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const openDeleteModal = (id, filename) => {
    if (!hasWriteAccess) {
      addNotification('error', t('components:techStackDataSources.noPermissionDelete'));
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        addNotification('success', t('components:techStackDataSources.deleteSuccess', { filename }));
      } else {
        addNotification('error', t('components:techStackDataSources.deleteError', { filename, error: response.error }));
      }
    } catch (error) {
      console.error('Error deleting file:', error);
      addNotification('error', t('components:techStackDataSources.deleteError', { filename: fileToDelete.filename }));
    } finally {
      setLoading(false);
  ...
  const columnDefinitions = [
    {
      id: 'filename',
      header: t('components:techStackDataSources.filename'),
      cell: item => item.filename,
      sortingField: 'filename'
    },
    {
      id: 'timestamp',
      header: t('components:techStackDataSources.timestamp'),
      cell: item => formatTimestamp(item.timestamp),
      sortingField: 'timestamp'
    },
    {
      id: 'fileFormat',
      header: t('components:techStackDataSources.fileFormat'),
      cell: item => item.fileFormat,
      sortingField: 'fileFormat'
    },
    {
      id: 'status',
      header: t('components:techStackDataSources.status'),
      cell: item => {
        ...
        switch (status.toLowerCase()) {
          case 'processed':
            ...
            statusText = t('components:techStackDataSources.processed');
            statusColor = 'text-status-success';
            break;
          case 'processing':
            statusIcon = 'status-in-progress';
            statusText = t('components:techStackDataSources.processing');
            statusColor = 'text-status-info';
            break;
          case 'failed':
            statusIcon = 'status-negative';
            statusText = t('components:techStackDataSources.failed');
            statusColor = 'text-status-error';
            break;
          case 'pending':
          default:
            statusIcon = 'status-pending';
            statusText = t('components:techStackDataSources.pending');
    ...
    {
      id: 'download',
      header: t('components:techStackDataSources.download'),
      cell: item => (
        <Button
          variant="icon"
          iconName="download"
          ariaLabel={t('components:techStackDataSources.downloadFile', { filename: item.filename })}
    ...
    {
      id: 'delete',
      header: t('components:techStackDataSources.delete'),
      cell: item => {
        const deleteButton = (
          <Button
            variant="icon"
            iconName="remove"
            ariaLabel={t('components:techStackDataSources.deleteFile', { filename: item.filename })}
        ...
        return hasWriteAccess ? (
          ...
          <Popover
            ...
            position="top"
            size="small"
            content={t('components:techStackDataSources.noPermissionTooltip')}
          >
            {deleteButton}
          </Popover>
        );
    ...
    try {
      ...
      if (response.success) {
        ...
        if (hasDataChanged(currentDataRef.current, newData)) {
          console.log('📊 [TechStackDataSourcesTable] data changed, updating UI');
          ...
          if (onDataChanged) {
            console.log('🔔 [TechStackDataSourcesTable] Notifying parent of data change');
            onDataChanged();
          }
        } else {
          console.log('✓ [TechStackDataSourcesTable] data unchanged, skipping UI update');
        }
      } else {
        console.error('Error loading data sources:', response.error);
        addNotification('error', t('components:techStackDataSources.loadError', { error: response.error }));
        setAllDataSources([]);
      ...
      addNotification('error', t('components:techStackDataSources.loadError'));
      setAllDataSources([]);
      setFilteredDataSources([]);
      setTotalItems(0);
    } finally {
  ...
  useEffect(() => {
    if (refreshTrigger && refreshTrigger !== prevRefreshTrigger.current) {
      console.log('🔔 External refresh triggered for TechStackDataSourcesTable');
  ...
  return (
    <>
      ...
      <Table
        ...
        loading={loading}
        loadingText={t('components:techStackDataSources.loadingDataSources')}
        ...
        empty={
          <Box textAlign="center" color="inherit">
            <b>{t('components:techStackDataSources.noDataSources')}</b>
            <Box padding={{ bottom: "s" }} variant="p" color="inherit">
              {t('components:techStackDataSources.noDataSourcesToDisplay')}
            </Box>
          </Box>
        }
        header={
          ...
          >
            {t('components:techStackDataSources.dataSources')}
          </Header>
        }
        filter={
          <div style={{ maxWidth: '300px' }}>
            <TextFilter
              ...
              filteringPlaceholder={t('components:techStackDataSources.findByFilename')}
              filteringAriaLabel={t('components:techStackDataSources.filterByFilename')}
        ...
        preferences={
          <CollectionPreferences
            title={t('components:techStackDataSources.preferences')}
            confirmLabel={t('common:confirm')}
            cancelLabel={t('common:cancel')}
            preferences={preferences}
            pageSizePreference={{
              title: t('components:techStackDataSources.pageSize'),
              options: [
                { value: 5, label: t('components:techStackDataSources.fiveFiles') },
                { value: 10, label: t('components:techStackDataSources.tenFiles') },
                { value: 20, label: t('components:techStackDataSources.twentyFiles') }
              ]
            }}
            visibleContentPreference={{
              title: t('components:techStackDataSources.selectVisibleColumns'),
              options: [
                {
                  label: t('components:techStackDataSources.fileProperties'),
                  options: [
                    { id: "filename", label: t('components:techStackDataSources.filename') },
                    { id: "timestamp", label: t('components:techStackDataSources.timestamp') },
                    { id: "fileFormat", label: t('components:techStackDataSources.fileFormat') },
                    { id: "status", label: t('components:techStackDataSources.status') },
                    { id: "download", label: t('components:techStackDataSources.download') },
                    { id: "delete", label: t('components:techStackDataSources.delete') }
                  ]
                }
              ]
            }}
      ...
      <Modal
        ...
        header={t('components:techStackDataSources.confirmDeletion')}
        ...
        {fileToDelete && (
          <SpaceBetween size="m">
            <Box variant="span" dangerouslySetInnerHTML={{
              __html: t('components:techStackDataSources.confirmDeleteMessage', { filename: fileToDelete.filename })
            }} />
            <Alert type="warning">
              {t('components:techStackDataSources.actionCannotBeUndone')}
            </Alert>
          </SpaceBetween>
        )}
      </Modal>
...
export default TechStackDataSourcesTable;
