...
function UtilizationDataSourcesTable({ dataSourceType = 'applications-utilization', refreshTrigger = 0, projectId, onDataProcessingComplete, onDataChanged }) {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (hasDataChanged(currentDataRef.current, newData)) {
          console.log('📊 [UtilizationDataSourcesTable] data changed, updating UI');
          ...
          if (onDataChanged) {
            console.log('🔔 [UtilizationDataSourcesTable] Notifying parent of data change');
            onDataChanged();
          }
        } else {
          console.log('✓ [UtilizationDataSourcesTable] data unchanged, skipping UI update');
        }
      } else {
      ...
      addNotification('error', t('components:utilizationDataSources.loadError', { error: '' }));
      setAllDataSources([]);
      setFilteredDataSources([]);
      setTotalItems(0);
    } finally {
  ...
  useEffect(() => {
    if (refreshTrigger && refreshTrigger !== prevRefreshTrigger.current) {
      console.log('🔔 External refresh triggered for UtilizationDataSourcesTable');
    ...
    <>
      ...
      >
        {fileToDelete && (
          <SpaceBetween size="m">
            <Box variant="span">
              {t('components:utilizationDataSources.confirmDeleteMessage', { filename: fileToDelete.filename })}
            </Box>
            <Alert type="warning">
...
export default UtilizationDataSourcesTable;
