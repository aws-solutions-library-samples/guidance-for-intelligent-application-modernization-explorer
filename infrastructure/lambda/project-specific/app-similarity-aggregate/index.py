"""
App Similarity Aggregate Lambda Function
...
def handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        all_similarities = []
        total_processed_applications = 0
        ...
        for partition_idx, partition_result in enumerate(partition_results):
            print(f"📦 Processing partition {partition_idx + 1}/{len(partition_results)}")
            ...
            if status_code == 200:
                ...
                applications_in_partition = partition_result.get('processed_applications', 0)
                total_processed_applications += applications_in_partition
                
                print(f"   ✅ Successful partition with {applications_in_partition} applications processed")
        ...
        print(f"   🔢 Total applications processed: {total_processed_applications}")
        print(f"   🔗 Total similarity records found: {len(all_similarities)}")
        
        # Store aggregated results in S3 to avoid Step Functions payload size limit
        timestamp = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
        aggregated_s3_key = f"app-aggregated-results/{PROJECT_ID}/aggregated_{timestamp}.json"
        
        print(f"💾 Storing aggregated results in S3...")
        ...
        aggregated_data = {
            ...
            'metadata': {
                ...
                'total_processed_applications': total_processed_applications,
        ...
        print(f"📤 Returning S3 reference to avoid Step Functions payload size limit")
