...
function combineScores(application, criteria) {
  ...
  // New format: pilot_score.total, Old format: algorithmicScore
  const algorithmicScore = application.pilot_score?.total || application.algorithmicScore || 0;
  ...
  return {
    ...
    // Detailed breakdowns - support both formats
    algorithmicBreakdown: application.scoreBreakdown || application.pilot_score || {},
    aiInsights: {
...
async function storeAllResults(projectId, jobId, application, consolidated) {
  ...
  const ruleBased = {
    ...
    scoreBreakdown: application.pilot_score || application.scoreBreakdown || {}
  };
