...
# App-ModEx Prompt Templates Stack Deployment Script
# Deploys the Prompt Templates DynamoDB stack
...
show_usage() {
    ...
    echo ""
    echo "Deploys the App-ModEx Prompt Templates Stack"
    echo ""
    echo "Options:"
...
print_status "Deploying Prompt Templates Stack"
...
LAMBDA_DIR="$INFRASTRUCTURE_DIR/lambda/global/seed-prompts"
if [[ -d "$LAMBDA_DIR" ]]; then
    if [[ ! -d "$LAMBDA_DIR/node_modules" ]]; then
        print_status "Installing seed-prompts Lambda dependencies..."
        cd "$LAMBDA_DIR"
        npm install
        cd "$INFRASTRUCTURE_DIR"
    fi
...
# Deploy only the Prompt Templates stack
print_status "Deploying Prompt Templates stack..."
npx cdk deploy "AppModEx-PromptTemplates" \
    --context environment="$ENVIRONMENT" \
    --region "$REGION" \
    --require-approval never

if [[ $? -eq 0 ]]; then
    print_success "Prompt Templates Stack deployed successfully!"
else
    print_error "Prompt Templates Stack deployment failed"
    exit 1
fi

print_success "Prompt Templates Stack deployment completed!"
