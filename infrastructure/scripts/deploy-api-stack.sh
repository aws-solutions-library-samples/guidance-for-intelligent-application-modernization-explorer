...
# App-ModEx API Stack Deployment Script
# Deploys API Gateway and authorizers
...
show_usage() {
    ...
    echo ""
    echo "Deploys the App-ModEx API Stack (API Gateway)"
    echo ""
    echo "Options:"
...
print_status "Deploying API stack..."
npx cdk deploy "AppModEx-Api" \
    --context environment="$ENVIRONMENT" \
    --region "$REGION" \
    --require-approval never

if [[ $? -eq 0 ]]; then
    print_success "API Stack deployed successfully!"
else
    print_error "API Stack deployment failed"
    exit 1
fi

print_success "API Stack deployment completed!"
